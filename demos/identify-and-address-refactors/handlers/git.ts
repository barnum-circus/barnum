...
export const createWorktree = createHandler(
  {
    ...
    handle: async ({ value }) => {
      ...
      console.error(
        `[create-worktree] Creating worktree: ${value.branch} at ${worktreePath}`,
      );
      ...
      git(
        ["-C", repoRoot, "worktree", "add", worktreePath, "-b", value.branch],
        repoRoot,
      );
...
export const deleteWorktree = createHandler(
  {
    ...
    handle: async ({ value }) => {
      ...
      try {
        ...
        console.error(
          `[delete-worktree] Warning: worktree removal failed, cleaning up manually`,
        );
...
export const createPR = createHandler(
  {
    ...
    handle: async ({ value }) => {
      console.error(
        `[create-pr] Would create PR for ${value.branch}: ${value.title}`,
      );
