...
export type ClassifyJudgmentResult = TaggedUnion<
  "ClassifyJudgmentResult",
  ClassifyJudgmentResultDef
>;
...
export const analyze = createHandler(
  {
    ...
    handle: async ({ value }): Promise<Refactor[]> => {
      console.error(
        `[analyze] Analyzing ${value.file} for refactoring opportunities...`,
      );
      ...
      try {
        ...
        console.error(
          `[analyze] Found ${refactors.length} opportunities in ${value.file}`,
        );
        return refactors;
      } catch {
        console.error(
          `[analyze] Failed to parse Claude's response, returning empty`,
        );
        return [];
      }
    },
...
export const assessWorthiness = createHandler(
  {
    ...
    handle: async ({ value: refactor }): Promise<Option<Refactor>> => {
      ...
      try {
        ...
        console.error(
          `[assess-worthiness] Skipping: ${result.reason ?? "not worth it"}`,
        );
        return { kind: "Option.None", value: null };
      } catch {
        // If we can't parse, include it (demo safety)
        console.error(
          "[assess-worthiness] Could not parse response, including by default",
        );
        return { kind: "Option.Some", value: refactor };
      }
    },
...
export const deriveBranch = createHandler(
  {
    ...
    handle: async ({ value }) => {
      console.error(
        `[derive-branch] Deriving branch name from: ${value.description.slice(0, 60)}`,
      );
      return {
        branch: `refactor/${value.description
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")
          .slice(0, 40)}`,
      };
...
export const preparePRInput = createHandler(
  {
    ...
    handle: async ({ value }) => {
      console.error(
        `[prepare-pr-input] Preparing PR for branch ${value.branch}`,
      );
      return {
        branch: value.branch,
...
export const implement = createHandler(
  {
    ...
    handle: async ({ value }) => {
      console.error(
        `[implement] Applying refactor in ${value.worktreePath}: ${value.description}`,
      );
...
export const commit = createHandler(
  {
    ...
    handle: async ({ value }) => {
      ...
      const result = spawnSync(
        "git",
        ["commit", "-m", "Apply automated refactor", "--allow-empty"],
        {
          cwd: value.worktreePath,
          encoding: "utf-8",
        },
      ...
      if (result.status !== 0) {
        console.error(
          `[commit] Warning: commit may have failed: ${result.stderr}`,
        );
      } else {
...
export const judgeRefactor = createHandler(
  {
    ...
    handle: async (): Promise<JudgmentResult> => {
      ...
      try {
        ...
        console.error(
          `[judge-refactor] ${judgment.approved ? "Approved" : "Needs work"}`,
        );
        return judgment;
      } catch {
        // If we can't parse, approve it (demo safety)
        console.error(
          "[judge-refactor] Could not parse response, approving by default",
        );
...
export const classifyJudgment = createHandler(
  {
    ...
    outputValidator: taggedUnionSchema("ClassifyJudgmentResult", {
      Approved: z.null(),
      NeedsWork: z.string(),
    }),
    handle: async ({ value: judgment }): Promise<ClassifyJudgmentResult> => {
      console.error(
        `[classify-judgment] Classifying judgment (approved=${judgment.approved})`,
      );
      ...
      return {
        kind: "ClassifyJudgmentResult.NeedsWork",
        value: judgment.instructions,
      };
    },
  },
  "classifyJudgment",
