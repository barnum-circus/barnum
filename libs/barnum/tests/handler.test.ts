...
describe("createHandler optional types", () => {
  ...
  it("rejects handle that returns wrong type for explicit TOutput", () => {
    ...
    createHandler<string, number>({ handle: async ({ value }) => value.toUpperCase() }, "handler");
  });
  ...
  it("rejects inputValidator that contradicts explicit TValue", () => {
    ...
    createHandler<string, number>({ inputValidator: z.number(), handle: async ({ value }) => value.length }, "handler");
  });

  it("rejects outputValidator that contradicts explicit TOutput", () => {
    // @ts-expect-error — TOutput is number but validator is z.string()
    createHandler<string, number>({ inputValidator: z.string(), outputValidator: z.string(), handle: async ({ value }) => value.length }, "handler");
  });
  ...
  it("rejects outputValidator that contradicts inferred TOutput", () => {
    ...
    createHandler({ inputValidator: z.string(), outputValidator: z.string(), handle: async ({ value }) => value.length }, "handler");
  });
  ...
  it("rejects inputValidator that contradicts handle parameter", () => {
    ...
    createHandler({ inputValidator: z.number(), handle: async ({ value }) => value.toUpperCase() }, "handler");
  });
  ...
  it("rejects inputValidator wider than explicit TValue", () => {
    ...
    createHandler<"hello", string>({ inputValidator: z.string(), handle: async ({ value }) => value }, "handler");
  });

  it("rejects outputValidator wider than explicit TOutput", () => {
    // @ts-expect-error — TOutput is "ok" but validator accepts any string
    createHandler<string, "ok">({ inputValidator: z.string(), outputValidator: z.string(), handle: async ({ value }) => "ok" as const }, "handler");
  });
...
describe("createHandlerWithConfig optional types", () => {
  ...
  it("rejects wrong stepConfigValidator", () => {
    ...
    createHandlerWithConfig<never, string, { retries: number }>({ stepConfigValidator: z.string(), handle: async ({ stepConfig }) => String(stepConfig) }, "handler");
  });
  ...
  it("rejects outputValidator contradicting handle return", () => {
    // @ts-expect-error — handle returns number, outputValidator is z.string()
    createHandlerWithConfig({ outputValidator: z.string(), handle: async ({ stepConfig }) => 42 }, "handler");
  });
  ...
  it("rejects stepConfigValidator wider than explicit TStepConfig", () => {
    ...
    createHandlerWithConfig<never, string, { retries: 3 }>({ stepConfigValidator: z.object({ retries: z.number() }), handle: async ({ stepConfig }) => String(stepConfig.retries) }, "handler");
  });
