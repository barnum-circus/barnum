...
Five node types. Handle absorbs All (concurrent bodies), ForEach (dynamic concurrent bodies), Branch (case-dispatch handler), and Loop/tryCatch/race (RestartHandle + body-level Branch for routing). Invoke becomes ResumePerform targeting a root ResumeHandle.
