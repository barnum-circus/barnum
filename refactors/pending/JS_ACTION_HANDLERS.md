...
## Prerequisite Rust Changes (both done)

1. ~~**Always serialize `Step.next`:**~~ Done — `Step.next` in `resolved.rs` has `#[serde(default)]` with no `skip_serializing_if`, so it always serializes.

2. ~~**Generate resolved type schemas:**~~ Done (SCHEMA_GENERATION_CLEANUP). `barnum-resolved-schema.zod.ts` is generated by `build_schemas` and exports TypeScript types: `Config`, `Step`, `ActionKind`, `PoolAction`, `CommandAction`, `Options`, `Task`. Note: only the root `resolvedTypesSchema` is exported as a Zod schema object; the individual types are exported as `type` only (TypeScript types, not Zod validators).
