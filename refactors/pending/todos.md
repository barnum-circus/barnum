...
0. **Consistent `kind` Discriminator on All Tagged Unions** — The `MaybeLinked` type (used for `instructions`, `value_schema`) uses `{"inline": ...}` / `{"link": ...}` key-based discrimination instead of a `kind` field like everything else (`ActionFile`, `FinallyHook`, `PreHook`, `PostHook`). Change `MaybeLinked` to use `{"kind": "Inline", "value": "..."}` / `{"kind": "Link", "path": "..."}` (or similar) so every tagged union in the config schema uses the same `kind`-discriminator pattern. This is a serde `#[serde(tag = "kind")]` change.

0. **BUG: `queue_task` Bypasses FIFO Ordering** — When a task completes and spawns children, `task_succeeded` (line 699) decrements `in_flight`, then calls `queue_task` for each child. Since `in_flight` was just decremented, `queue_task` sees `in_flight < max_concurrency` and dispatches the child **immediately**, jumping ahead of all older Pending tasks. This means when ListFiles fans out 30 Analyze tasks and max_concurrency=3, each file's entire pipeline (Analyze → ScheduleNext → Refactor → Commit → ...) completes before later files' Analyze tasks even start. Files are effectively processed sequentially in groups of `max_concurrency`, not concurrently. **Fix:** `queue_task` should always insert as Pending. Let `dispatch_all_pending` (which iterates the BTreeMap in task ID order = FIFO) handle all dispatching. File: `crates/barnum_config/src/runner/mod.rs`, functions `queue_task` (line 414) and `task_succeeded` (line 698).
