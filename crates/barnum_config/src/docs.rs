...
use crate::resolved::{ActionKind, Config, PoolAction, Step};
use std::fmt::Write;
...
mod tests {
    ...
    fn generates_step_docs() {
        let config_file: ConfigFile = serde_json::from_str(
            r#"{
            "steps": [
                {"name": "Start", "action": {"kind": "Pool", "params": {"instructions": {"kind": "Inline", "value": "Begin here."}}}, "next": ["End"]},
                {"name": "End", "action": {"kind": "Pool", "params": {"instructions": {"kind": "Inline", "value": ""}}}, "next": []}
            ]
        }"#,
        )
        .unwrap();
    ...
    fn generates_full_docs() {
        let config_file: ConfigFile = serde_json::from_str(
            r#"{
            "steps": [
                {"name": "Start", "action": {"kind": "Pool", "params": {"instructions": {"kind": "Inline", "value": "Begin."}}}, "next": ["End"]},
                {"name": "End", "action": {"kind": "Pool", "params": {"instructions": {"kind": "Inline", "value": ""}}}, "next": []}
            ]
        }"#,
        )
