//! Macros for creating typed newtype wrappers around interned `StringKey`s.

/// Define a newtype wrapper around `StringKey` with standard derives, serde, and Display.
#[macro_export]
macro_rules! string_key_newtype {
    ($(#[$meta:meta])* $named:ident) => {
        $(#[$meta])*
        ...
        impl $named {
            /// Convert to another string key newtype without checking type identity.
...
/// Create a one-way `From` conversion and `PartialEq` between two string key newtypes.
#[macro_export]
...
/// Enable bidirectional `PartialEq` between two string key newtypes.
#[macro_export]
