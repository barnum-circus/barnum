//! Shared test utilities for troupe integration tests.
...
use troupe::{Response, STATUS_FILE, VerifiedWatcher, default_root, id_to_path};
use uuid::Uuid;
...
/// Pools live in `/tmp/troupe/pools/<pool>/`.
...
///
/// Executes: `troupe submit_task --pool <pool> --data <payload_json> --notify <method>`
...
impl From<troupe::DaemonConfig> for DaemonConfig {
    fn from(config: troupe::DaemonConfig) -> Self {
        Self {
...
impl TroupeHandle {
    /// Start the troupe daemon with default configuration.
    pub fn start(pool: &str, test_name: &str) -> Self {
        Self::start_with_config(pool, DaemonConfig::new(), test_name)
    }

    /// Start the troupe daemon with custom configuration.
